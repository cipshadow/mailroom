...
### Changed
- Dashboard's send button is now **Review & send**, not **Send now** —
  the old label promised an immediate action even though it opens the
  review screen (0.4.4) first.
...
### Changed
- **Clearer "Gmail API not enabled" error.** Forgetting to click Enable on
  the Gmail API page is the single most common first-run mistake (caught
  live during a fresh-install test) - the setup wizard used to surface
  Google's raw, nested error JSON for this case. It now shows a short,
  actionable message with a direct link built from the project number
  Google's own error names, telling you exactly what to click and how to
  confirm it worked (the button changing from "Enable" to "Manage").
- `docs/google-cloud-setup.md`: the Enable-the-API step now spells out what
  success looks like and what the failure error means, instead of a single
  line easy to skim past - this exact mistake happened even with the
  previous wording.
